...
/// Converts a fractional utilization (0.0–1.0) to a percentage (0–100).
func utilizationToPercent(_ utilization: Double) -> Int {
    return min(100, Int((utilization * 100).rounded()))
}
    ...
    func fetchUsage() {
        ...
        Task {
            do {
                ...
                // FIX: utilization is returned as a fraction (0.0–1.0),
                // must multiply by 100 to get a percentage.
                let fiveHourUtil = utilizationToPercent(response.fiveHour?.utilization ?? 0)
                let sevenDayUtil = utilizationToPercent(response.sevenDay?.utilization ?? 0)
                let sonnetUtil: Int? = response.sevenDaySonnet.map { utilizationToPercent($0.utilization) }
